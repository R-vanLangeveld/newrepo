...
validate.addClassificationRules = () => {
	return [
		body("classification_name").trim().escape().notEmpty().withMessage("A valid classification name is required.").custom(async (classification_name) => {
...
validate.checkUpdateData = async (req, res, next) => {
	...
	if (!errors.isEmpty()) {
		let nav = await utilities.getNav();
		res.render("inventory/edit-inventory", {errors, title: `Edit ${itemName}`, nav, classificationList: await utilities.buildClassificationList(), classification_id, inv_id, inv_make, inv_model, inv_description, inv_image, inv_thumbnail, inv_price, inv_year, inv_miles, inv_color});
		return
	}
	next();
}
